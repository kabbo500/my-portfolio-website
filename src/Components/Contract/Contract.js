import "./Contract.css";
import Phone from "../img/phone.png";
import Email from "../img/email.png";
import Address from "../img/address.png";
import React, { useRef, useState } from "react";
import emailjs from '@emailjs/browser';

const Contract = () => {
    const formRef = useRef();
    const [done, setDone] = useState(false)



    const handleSubmit = (e) => {
        e.preventDefault();
        emailjs.sendForm('service_rl8mfrx', 'template_fl5ep9p', formRef.current, 'A8L52gLJY10ykpLKT')
            .then((result) => {
                console.log(result.text);
                setDone(true)
            }, (error) => {
                console.log(error.text);
            });
    };


    return (
        <div className="c">
            <div className="c-bg"></div>
            <div className="c-wrapper">
                <div className="c-left">
                    <h1 className="c-title">Let's discuss your project</h1>
                    <div className="c-info">
                        <div className="c-info-item">
                            <img src={Phone} alt="" className="c-icon" />
                            +8801781184894
                        </div>
                        <div className="c-info-item">
                            <img className="c-icon" src={Email} alt="" />
                            kabbokazi3779@gmail.com
                        </div>
                        <div className="c-info-item">
                            <img className="c-icon" src={Address} alt="" />
                            Faridpur, Bangladesh
                        </div>
                    </div>
                </div>
                <div className="c-right">
                    <h1 className="c-desc">
                        <b>What's your story?</b>Contract With me.
                    </h1>
                    <form ref={formRef} onSubmit={handleSubmit}>
                        <input className="text-white" style={{ backgroundColor: "#333" }} type="text" placeholder="Name" name="user_name" />
                        <input className="text-white" style={{ backgroundColor: "#333" }} type="text" placeholder="Subject" name="user_subject" />
                        <input className="text-white" style={{ backgroundColor: "#333" }} type="text" placeholder="Email" name="user_email" />
                        <textarea className="text-white" style={{ backgroundColor: "#333" }} rows="5" placeholder="Message" name="message" />
                        <button>Submit</button>
                        {done && "Thank you for Contract with me"}
                    </form>
                </div>
            </div>
        </div>
    );
};

export default Contract;